...
program
  ...
  .action((opts: { src: string; target?: string; root: string }) => {
    ...
    for (const m of missing) {
      console.log(`! target "${m}" is declared but not implemented yet (planned: codex, dsh, cursor…)`);
    }
...
program
  ...
  .action((opts: { src: string; target?: string; root: string; write?: boolean; force?: boolean }) => {
    const loaded = load(opts.src);
    if (!loaded) return;
    const { ir, diagnostics } = loaded;
    printDiags(diagnostics);
    ...
    const { chosen, missing } = chooseTargets(ir, opts.target);
    for (const m of missing) {
      console.log(`! target "${m}" is declared but not implemented yet — skipped`);
    }
